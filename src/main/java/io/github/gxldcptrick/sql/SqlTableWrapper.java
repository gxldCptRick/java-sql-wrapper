...
//				.collect(Collectors.toList());
		...
		// crafting the update template im actually going to cry out of pain.. WHY GOD
		// WHY!!
		...
		// crafting the delete statement from scratch hardest one to do yet oof big oof
		// biggest oof
		...
		// crafting the complex select some template no joke this was actually tricky to
		// get right.
		var subQuery = String.format("SELECT TOP(?) id FROM %s ORDER BY id;", getTableName());
		selectSomeTemplate = String.format("SELECT TOP(?) %s FROM %s WHERE id NOT IN (%s) ORDER BY id;", propNameString,
				getTableName(), subQuery);
...
//        joinOnIDTemplate = String.format("SELECT %s FROM %s as %s1 INNER JOIN %s as %s2 ON %s1.%s = %s2.id", propNameString, getTableName(), getTableName() );
		/*
		 * Base join statement: SELECT %s FROM %s as %s1 INNER JOIN %s as %s2 ON
		 * %s1.COLUMN = %s2.id
		 */
//        System.out.println(selectTemplate);
	...
	private String createPropNames(Class<?> typeInfo) {
		return getFields(typeInfo).map(Field::getName).filter((c) -> !c.equals("id"))
				.reduce((agg, next) -> agg + ", " + next).orElse("");
	}
	...
	public void create(T instance) throws SQLException {
		...
		var fields = getFields(classOfInstance).filter(f -> !f.getName().equalsIgnoreCase("id"))
				.collect(Collectors.toList());
	...
	public void update(int id, T instance) throws SQLException {
		var fields = getFields(instance.getClass()).filter(f -> !f.getName().equalsIgnoreCase("id"))
				.collect(Collectors.toList());
	...
	private Method getMethodFromInstance(T instance, Field field, String getterOrSetter) {
		...
		Method getter;
		try {
			getter = classOfInstance.getMethod(getterName,
					(getterOrSetter.equalsIgnoreCase("get") ? new Class<?>[0] : new Class<?>[] { field.getType() }));
